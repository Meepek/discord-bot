...
def save_suggestion(user_id, username, category, description, reason, thread_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO suggestions (user_id, username, category, description, reason, thread_id) VALUES (?, ?, ?, ?, ?, ?)',
                   (user_id, username, category, description, reason, thread_id))
    conn.commit()
    conn.close()
...
def save_bug_report(user_id, category, bug_type, description, evidence, thread_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO bug_reports (user_id, category, bug_type, description, evidence, thread_id) VALUES (?, ?, ?, ?, ?, ?)',
                   (user_id, category, bug_type, description, evidence, thread_id))
    conn.commit()
    conn.close()
...
def save_complaint(user_id, complaint_type, target_user, data, thread_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO complaints (user_id, complaint_type, target_user, data, thread_id) VALUES (?, ?, ?, ?, ?)',
...
def save_appeal(user_id, appeal_type, data, thread_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO appeals (user_id, appeal_type, data, thread_id) VALUES (?, ?, ?, ?)',
...
def save_application(user_id, username, app_type, data, thread_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('INSERT INTO applications (user_id, username, application_type, data, thread_id) VALUES (?, ?, ?, ?, ?)',
...
def has_permission_for_type(user: discord.Member, item_type: str) -> bool:
    if "jb" in item_type.lower():
        return has_jb_permissions(user)
    elif "discord" in item_type.lower() or "dc" in item_type.lower():
        return has_discord_permissions(user)
...
class CommissionManagementSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label="Przyjmij do realizacji", value="accepted", emoji="🔧"),
...
async def create_generic_post(modal: discord.ui.Modal, interaction: discord.Interaction, post_type: str, emoji: str):
    ...
    try:
        ...
        if post_type.startswith("Propozycja"):
            save_suggestion(str(interaction.user.id), interaction.user.display_name, post_type, data.get('Opis propozycji'), data.get('Dlaczego ma zostać wprowadzona?'), str(thread_message.thread.id))
        elif post_type.startswith("Błąd"):
            save_bug_report(str(interaction.user.id), post_type, "Nieokreślony", data.get('Opis błędu'), data.get('Dowody (linki do screenów, filmów)'), str(thread_message.thread.id))
        elif post_type.startswith("Skarga"):
            save_complaint(str(interaction.user.id), post_type, data.get('Nick osoby, na którą składasz skargę'), data, str(thread_message.thread.id))
        elif post_type.startswith("Odwołanie"):
            save_appeal(str(interaction.user.id), post_type, data, str(thread_message.thread.id))
        elif post_type.startswith("Podanie"):
            save_application(str(interaction.user.id), interaction.user.display_name, post_type, data, str(thread_message.thread.id))
...
class ForumSelect(discord.ui.Select):
    def __init__(self, view_type: str, custom_id: str):
        ...
        if view_type == "proposals_bugs":
            placeholder, options = "Wybierz akcję (propozycje i błędy)...", [
                discord.SelectOption(label="Propozycja JB", emoji="💡", value="Propozycja JB"),
                discord.SelectOption(label="Propozycja DC", emoji="💡", value="Propozycja DC"),
                discord.SelectOption(label="Błąd JB", emoji="🐛", value="Błąd JB"),
                discord.SelectOption(label="Błąd DC", emoji="🐛", value="Błąd DC")]
        elif view_type == "complaints_appeals":
            placeholder, options = "Wybierz akcję (skargi i odwołania)...", [
                discord.SelectOption(label="Skarga JB", emoji="⚠️", value="Skarga JB"),
                discord.SelectOption(label="Skarga DC", emoji="⚠️", value="Skarga DC"),
                discord.SelectOption(label="Odwołanie JB", emoji="🔓", value="Odwołanie JB"),
                discord.SelectOption(label="Odwołanie DC", emoji="🔓", value="Odwołanie DC")]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        modal_map = {
            ...
            "Podanie Admin JB": AdminApplicationModal, "Podanie Zaufany JB": TrustedApplicationModal,
