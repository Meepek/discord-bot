...
# --- TYPY REKRUTACJI ---
# Punkt 3: Podanie Admin (wszystkie serwery) i Podanie Zaufany (tylko JB)
ADMIN_APPLICATION_SERVERS = [
    "JailBreak",
    "Supermoce",
    "Surf + RPG",
    "DeathRun",
...
# --- PUNKT 2: MAPOWANIE TAGÓW ---
# Tagi które bot będzie szukał na forum i automatycznie przypisywał do postów
TAG_MAPPING = {
    # Propozycje
...
async def init_database():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Dodaj kolumnę server do complaints i appeals jeśli nie istnieje
        for table in ["complaints", "appeals"]:
            try:
                await db.execute(f"ALTER TABLE {table} ADD COLUMN server TEXT DEFAULT 'Nieokreślony'")
            except Exception:
                pass
...
# --- POMOCNICZA FUNKCJA SZUKANIA TAGÓW (PUNKT 2) ---
def find_tag(forum_channel, tag_name: str):
    """
    Szuka tagu na kanale forum.
    Najpierw dokładne dopasowanie, potem częściowe.
    """
    # Dokładne dopasowanie
    tag = discord.utils.get(forum_channel.available_tags, name=tag_name)
    if tag:
        return tag
    # Częściowe dopasowanie (tag_name zawiera się w nazwie tagu lub odwrotnie)
...
def get_tag_name_for_post(post_type: str, server: str = None) -> str:
    """
    Zwraca nazwę tagu do wyszukania na podstawie typu postu i serwera.
    """
    if server and server != "Nieokreślony":
        # Dla propozycji, błędów, skarg, odwołań z serwerem
        ...
        # Dla podań (bez serwera)
        if post_type in TAG_MAPPING:
            return TAG_MAPPING[post_type]
        return post_type
...
# --- PUNKT 1: SKARGI I ODWOŁANIA Z WYBOREM SERWERA ---
class ComplaintModal(discord.ui.Modal):
    def __init__(self, server: str):
        super().__init__(title=f"Nowa Skarga - {server}")
        self.server = server
        self.target_nick = discord.ui.TextInput(
            label="Nick osoby, na którą składasz skargę", required=True)
...
class AppealModal(discord.ui.Modal):
    def __init__(self, server: str):
        ...
        self.ban_reason = discord.ui.TextInput(
            label="Powód otrzymanej kary", required=True, max_length=1024)
        self.add_item(self.ban_reason)
        self.admin_nick = discord.ui.TextInput(
            label="Nick admina, który nałożył karę", required=True)
...
class ServerAdminApplicationModal(discord.ui.Modal):
    """Uniwersalny modal dla podań adminowskich na serwerach gier."""
...
class EventModal(discord.ui.Modal, title="Nowe wydarzenie"):
    ...
    datetime_input = discord.ui.TextInput(
        label="Data i godzina (DD.MM.RRRR HH:MM)",
        placeholder="np. 25.12.2025 18:00", required=True)
...
async def process_decision(interaction: discord.Interaction, original_interaction: discord.Interaction,
    ...
    try:
        ...
        await log_action(interaction.guild, f"Zarządzano postem: {action_details['text']}", interaction.user,
                         f"Post: {original_interaction.channel.mention}")
...
async def create_generic_post(modal: discord.ui.Modal, interaction: discord.Interaction,
    ...
    try:
        ...
        # --- PUNKT 2: AUTOMATYCZNE TAGI ---
        tag_name = get_tag_name_for_post(post_type, server)
        tag = find_tag(forum_channel, tag_name)

        # Jeśli nie znaleziono tagu z serwerem, spróbuj bez serwera
        if not tag and server:
            tag = find_tag(forum_channel, post_type)
...
class ForumSelect(discord.ui.Select):
    def __init__(self, view_type: str, custom_id: str):
        ...
        if view_type == "proposals_bugs":
            ...
            # PUNKT 1: skargi i odwołania tak samo jak propozycje/błędy
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # PUNKT 1: Skargi i odwołania - pokaż wybór serwera
        ...
        # PUNKT 3: Podanie Admin - pokaż wybór serwera
        ...
        if choice == "Podanie Zaufany JB":
            ...
            requirements = "• Minimum 14 lat\n• Aktywność na serwerze"
            embed = discord.Embed(title=f"📋 Wymagania - {choice}", description=requirements, color=COLORS["main"])
            if LOGO_URL:
                embed.set_thumbnail(url=LOGO_URL)
            embed.set_footer(text=FOOTER_TEXT)
...
# --- PUNKT 3: WYBÓR SERWERA DLA PODANIA ADMIN ---
...
class AdminApplicationServerSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Sprawdź czy rekrutacja otwarta
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT is_open FROM recruitment_status WHERE position = ?", (choice,)) as cursor:
                status = await cursor.fetchone()
                is_open = status[0] if status else 1
        ...
        if choice in requirements_map:
            embed = discord.Embed(title=f"📋 Wymagania - {choice}", description=requirements_map[choice], color=COLORS["main"])
            if LOGO_URL:
                embed.set_thumbnail(url=LOGO_URL)
            embed.set_footer(text=FOOTER_TEXT)
            await interaction.response.send_message(embed=embed, view=RequirementsView(choice), ephemeral=True)
        else:
            modal = ServerAdminApplicationModal(choice)
            await interaction.response.send_modal(modal)


# --- PUNKT 1: WYBÓR SERWERA DLA SKARG I ODWOŁAŃ ---
...
class ServerSelect(discord.ui.Select):
    def __init__(self, post_type: str):
        self.post_type = post_type
        options = [discord.SelectOption(label=server, value=server, emoji="🌐") for server in SERVER_LIST]
        super().__init__(placeholder="Wybierz serwer...", min_values=1, max_values=1, options=options)
...
class RequirementsView(discord.ui.View):
    def __init__(self, application_type: str):
        ...
        # Przycisk statystyk dla serwerów gier
        game_server_types = [
...
class ShopCategorySelect(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(
            placeholder="Wybierz kategorię sklepu...",
...
class ShopItemSelect(discord.ui.Select):
    def __init__(self, category: str):
        super().__init__(
            placeholder="Wybierz przedmiot, który chcesz kupić...",
            min_values=1, max_values=1,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute(
                "SELECT name, cost, role_id, stock, category FROM shop_items WHERE id = ?", (item_id,)) as cursor:
            ...
            async with db.execute(
                "SELECT points FROM reputation_points WHERE user_id = ?", (str(interaction.user.id),)) as cursor:
            ...
            await db.execute(
                "UPDATE reputation_points SET points = ? WHERE user_id = ?", (new_points, str(interaction.user.id)))
            if stock is not None:
                await db.execute("UPDATE shop_items SET stock = stock - 1 WHERE id = ?", (item_id,))
            if category == "Specjalne role":
                await db.execute(
                    "INSERT INTO shop_purchases (user_id, item_id) VALUES (?, ?)", (interaction.user.id, item_id))
            await db.commit()
        ...
        if role_id:
            try:
                ...
                if role_to_add:
                    ...
                    await interaction.followup.send(
                        f"✅ Gratulacje! Kupiłeś i otrzymałeś rolę **{item_name}** za **{item_cost}** reputacji. Twoje saldo: **{new_points}** rep.",
                        ephemeral=True)
                else:
                ...
                await interaction.followup.send(
                    f"✅ Zakupiono **{item_name}**, ale wystąpił błąd przy nadawaniu roli. Administracja została powiadomiona.",
                    ephemeral=True)
        else:
            await interaction.followup.send(
                f"✅ Gratulacje! Kupiłeś **{item_name}** za **{item_cost}** reputacji. Twoje saldo: **{new_points}** rep.\nAdministracja została powiadomiona.",
                ephemeral=True)
        ...
        if SHOP_CONFIG.get("channel_id") and category in ["VIP", "Premium", "Fajki", "Oferty Dnia"]:
            ...
            if notif_channel:
                ...
                embed = discord.Embed(title="🛒 Nowy zakup w sklepie!", color=COLORS["success"], timestamp=datetime.now(POLAND_TZ))
...
async def setup_przypomnienia(interaction: discord.Interaction, wlaczone: bool,
    ...
    await interaction.response.send_message(
        f"✅ Automatyczne przypomnienia zostały **{status}**. Czas oczekiwania: **{dni} dni**.", ephemeral=True)
...
async def setup_forum_rekrutacje(interaction: discord.Interaction, kanal_forum: discord.ForumChannel):
    ...
    await interaction.response.send_message(
        f"✅ Panel rekrutacyjny został utworzony na {kanal_forum.mention}!", ephemeral=True)
...
async def setup_forum_rekrutacje_kreatywne(interaction: discord.Interaction, kanal_forum: discord.ForumChannel):
    ...
    embed = discord.Embed(
        ...
        description="Chcesz dołączyć do ekipy kreatywnej? Wybierz stanowisko z menu poniżej.",
        color=COLORS["main"])
    if LOGO_URL:
        embed.set_thumbnail(url=LOGO_URL)
    embed.set_footer(text=FOOTER_TEXT)
    ...
    await interaction.response.send_message(
        f"✅ Panel rekrutacji kreatywnej został utworzony na {kanal_forum.mention}!", ephemeral=True)
...
async def setup_powiadomienia_sklep(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.response.send_message(
        f"✅ Skonfigurowano kanał powiadomień o zakupach na {kanal.mention}.", ephemeral=True)
...
async def dodaj_przedmiot(interaction: discord.Interaction, kategoria: str, nazwa: str,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await interaction.response.send_message(
        f"✅ Dodano przedmiot '{nazwa}' do kategorii '{kategoria}' za **{koszt}** reputacji.", ephemeral=True)
...
async def dodaj_specjalna_role(interaction: discord.Interaction, nazwa: str,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await interaction.response.send_message(
        f"✅ Dodano rolę {rola.mention} jako '{nazwa}' ({ilosc} szt.) za **{koszt}** reputacji.", ephemeral=True)
...
async def setup_sklep_panel(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.response.send_message(
        f"✅ Panel sklepu został utworzony na kanale {kanal.mention}.", ephemeral=True)
...
async def reputacja_dodaj(interaction: discord.Interaction, uzytkownik: discord.Member,
    ...
    await interaction.response.send_message(
        f"✅ Dodano **{ilosc}** reputacji dla {uzytkownik.mention}. Nowe saldo: **{new_balance}** rep.", ephemeral=True)
...
async def reputacja_usun(interaction: discord.Interaction, uzytkownik: discord.Member,
    ...
    new_balance = await update_reputation(uzytkownik.id, -ilosc, mode='add')
    await interaction.response.send_message(
        f"✅ Usunięto **{ilosc}** reputacji {uzytkownik.mention}. Nowe saldo: **{new_balance}** rep.", ephemeral=True)
...
async def rekrutacja_otworz(interaction: discord.Interaction, stanowisko: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await interaction.response.send_message(
        f"✅ Rekrutacja na **{stanowisko}** została **otwarta**.", ephemeral=True)
...
async def rekrutacja_zamknij(interaction: discord.Interaction, stanowisko: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await interaction.response.send_message(
        f"✅ Rekrutacja na **{stanowisko}** została **zamknięta**.", ephemeral=True)
